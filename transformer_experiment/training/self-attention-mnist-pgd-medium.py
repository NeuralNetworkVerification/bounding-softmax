...
if __name__ == '__main__':
    ...
    x = Reshape([4, 36])(x)
    attention = MultiHeadAttention(num_heads=3, key_dim=12)
    ...
    model.save('self-attention-mnist-pgd-medium.h5')

    exit()

    # reload the keras and onnx models to check if equivalent
    model = load_model('self-attention-mnist-pgd-medium.h5')
    model.summary()
    score = model.evaluate(x_test, y_test, verbose=0)
    print("Test loss:", score[0])
    print("Test accuracy:", score[1])
    ...
    import tf2onnx
    model_proto, _ = tf2onnx.convert.from_keras(model, output_path='self-attention-mnist-pgd-medium.onnx')

    import onnx
    import onnxruntime
    # onnx_model = onnx.load_model('self-attention-mnist-pgd-medium.onnx')
    session = onnxruntime.InferenceSession('self-attention-mnist-pgd-medium.onnx')
