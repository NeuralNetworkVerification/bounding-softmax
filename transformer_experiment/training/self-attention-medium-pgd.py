...
x = Flatten()(inp)
x = Dense(2*2*9*4, activation='relu')(x)

if True:
    x = Reshape([2*2, 9*4])(x)
    att = MultiHeadsAttModel(l=4, d=9*4, dv=3*4, dout=32, nv=3)
    x = att([x, x, x])
...
robust_model.save('med-pgd.h5')

#robust_model.layers[-1].activation = keras.activations.linear
#robust_model.save("robust_mnist_8x100_linear.h5")
robust_model = tf.keras.models.load_model('med-pgd.h5')

import keras2onnx
onnx_model = keras2onnx.convert_keras(robust_model, 'med-pgd')
keras2onnx.save_model(onnx_model, 'med-pgd.onnx')
