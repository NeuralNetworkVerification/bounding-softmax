...
NETWORK=./self-attention-med-sim.onnx
...
do
    ...
    do
        ...
        for i in {0..499}
        do
            sFile="$i"_"$e"_"$t"_med.summary
            $SCRIPT $NETWORK --dataset mnist -i $i -e $e --softmax-bound $t --summary-file="$sFile" --verbosity=0
            if [ ! -f $sFile ]; then
                echo $i $e $t misclassify >> results_med.csv
            elif grep -q unsat "$sFile"; then
                echo $i $e $t unsat >> results_med.csv
                rm $sFile
            else
                echo $i $e $t unknown >> results_med.csv
                rm $sFile
            fi
        done
    done
